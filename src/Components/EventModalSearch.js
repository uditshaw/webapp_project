...
export default function EventModalSearch(props) {
  ...
  {
    let k={}
    let data=await fetch("http://localhost:8000/api/v1/events/"+id,{method:'GET',  headers: {"Content-Type": "application/json"}}).then((res)=>{return res.json()}).then((d)=>k=d)
  ...
  return (
    <div>
      <Link underline='none' onClick={()=>{handleOpen(props.item._id)}}>
        <Card sx={card}>
            <CardMedia sx={cardmedia} component='img' image={props.item.image}></CardMedia>
            <CardContent sx={cardcontent}>
                <Typography variant='body1'><b>{props.item.name}</b></Typography>
                <Typography variant='body2'><i>({props.item.status})</i></Typography>
            </CardContent>
